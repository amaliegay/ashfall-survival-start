...
local function transferItems()
    ...
    tes3.messageBox(
        "Modded starting equipment has been added to your inventory.")
end

local function onWater(e)
    local cell = tes3.player.cell
    local waterLevel = cell.hasWater and cell.waterLevel
    if not cell.isInterior and waterLevel and e.reference.position.z -
        waterLevel < 30 then return true end
    return false
end

local function canSail(e) return tes3.player.data.ass.hasMap and onWater(e) end
...
local sailHome = {
    ...
    callback = function(e)
        if tes3.isModActive("TR_Mainland.esm") then
            tes3.positionCell({
                ...
                cell = "Bal Oyra",
                position = {152976, 202552, 94},
                orientation = {0, 0, 4.37}
            })
        else
            tes3.positionCell({
                reference = tes3.player,
                cell = "Dagon Fel",
                position = {61617, 183466, 36}
            })
        end
        transferItems()
    ...
    tooltipDisabled = function(e)
        return {
            text = onWater(e) and
                "You don't know which direction is Vvardenfell. Maybe there's a map on this island." or
...
local outdoorsOnly = {
    getLabel = function() return "Outdoors" end,
    check = function()
        local cell = tes3.player.cell
...
local function registerBushcraftingRecipe(e)
    ...
    local recipe = {
        ...
        materials = {
            {material = "wood", count = 14}, {material = "resin", count = 4},
            {material = "rope", count = 7}, {material = "straw", count = 1}
        },
